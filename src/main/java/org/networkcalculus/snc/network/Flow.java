...
package org.networkcalculus.snc.network;

import java.io.Serializable;

import java.util.List;

import org.networkcalculus.snc.gui.Displayable;
 ...
 * route through the network is given by an ArrayList of {@link Vertex}-ids. At
 * each vertice the flow has a priority given in the ArrayList
 * <code>priorities</code>. Further <code>
 * flow</code> has a third ArrayList, which consists of {@link
 * Arrival}s. Initially at most the first arrival in that array is given,
 * further arrivals are initialized by calculating output- bounds in the {
 * @Vertex}-class. Calculated arrivals are called established.
 * 
 * @author Michael Beck
...
public class Flow implements Serializable, Displayable {
     ...
     *
     * @param flow_ID
     * @param vertices the vertex-ids, through which the flow traverses the
     * network. vertices[0] denotes the id of the first vertex the flow
     * traverses.
     * @param arrivals the arrivals of the flow, including the initial arrival
     * at the first flow, given by arrivals[0]. Further arrivals might be
     * overwritten by invoking {@link
     * Analysis}.
     * @param priorities the priorities of the flows at the different vertices.
     * priorities[0] describes the priority of the flow at the first vertex. The
     * flows are served in descending priority numbers, in expression the flow
     * with the highest priority number is served first.
     * @param alias An optional alias, will be set to the empty string, if the
     * argument is null
    ...
    /**
     * Adds a node to this flow. A new dummy arrival is associated to this new
     * node.
     *
     * @param vertex_ID the node, which is added to the flow
     * @param priority the priority, which the flow has at this new node.
     */
    ...
    /**
     * The flow learns its next arrival. This means, that the next dummy arrival
     * is overwritten by the input and the number of established arrivals is
     * increased by one. If all arrivals are known nothing happens.
     *
     * @param arrival overwrites the next dummy arrival in the flow
     */
    public void learnArrival(Arrival arrival) {
        if (established_arrivals < arrivals.size()) {
    ...
    /**
     * Removes a vertex form this flow, i.e. removes it from the route together
     * with its corresponding priority and arrival entries. If the vertex
     * appears several times in the route all entries containing this vertex
     * will be deleted.
     *
     * @param vertexID
     */
    ...
    /**
     * Sets the initial arrival of the flow. This will overwrite any initial
     * arrivals, which may have been given before. Further the number of
     * established arrivals is set to 1. This means all arrivals, which may have
     * been established before are now again non-established and will be
     * overwritten by an analysis.
     *
    ...
    /**
     * returns the next vertex_id, for which its arrival is not yet established.
     *
     * @return the vertex_id of the vertex, which has the first non-established
     * arrival of this flow. Or -1 if there is no such flow.
     */
    public int getNextVertexID() {
        if (established_arrivals < vertices.size()) {
    ...
    /**
     * returns the vertext_id of the node, whose arrival was the last
     * established one.
     *
     * @return
     */
